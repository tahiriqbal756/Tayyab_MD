   ...
   if (pairingCode && !GlobalTechInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : 923074565864")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number 😂\nExample: 923074565864 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
      ...
      setTimeout(async () => {
         ...
         console.log(chalk.black(chalk.bgGreen(`Your Tayyab Md Pairing Code : `)), chalk.black(chalk.white(code)))
      }, 3000)
   }
    ...
    GlobalTechInc.ev.on('messages.upsert', async chatUpdate => {
        ...
        try {
            ...
            require(".Tayyab Md")(GlobalTechInc, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
